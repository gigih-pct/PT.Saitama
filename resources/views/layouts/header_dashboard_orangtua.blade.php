...
<head>
    ...
    <title>@yield('title', 'Dashboard Orang Tua') | PT Saitama</title>
    ...
    <header class="fixed top-0 left-0 w-full h-16 bg-[#173A67] flex items-center justify-between px-6 z-50 shadow-sm text-white">
        ...
        <div class="flex items-center gap-4">
            <div class="flex items-center gap-2">
                <span class="font-medium text-sm">{{ Auth::user()->name ?? 'Orang Tua' }}</span>
            ...
            <!-- Logout Button -->
            <form method="POST" action="{{ route('orangtua.logout') }}" class="flex items-center border-l border-white/20 pl-4 h-6">
                @csrf
