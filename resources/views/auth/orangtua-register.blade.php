<!-- resources/views/auth/orangtua-register.blade.php -->
<!DOCTYPE html>
<html lang="id">
<head>
    <title>Registrasi Orang Tua | PT Saitama Juara Dunia</title>
...
<div class="w-1/2 flex items-center justify-center">
    <div class="w-[420px] text-center">
        ...
        <p class="font-semibold mb-2">Daftar Sebagai:</p>
        <div class="flex justify-center gap-8 mb-6">
            <label class="flex items-center gap-2">
                <input type="radio" name="role" value="siswa" checked class="accent-red-500">
                <span>Siswa</span>
            </label>
            <label class="flex items-center gap-2">
                <input type="radio" name="role" value="orangtua" class="accent-red-500">
                <span>Orang Tua</span>
            </label>
        </div>
        ...
        <form method="POST" action="{{ route('orangtua.register.store') }}" class="space-y-4">
            @csrf
            ...
            <div class="flex justify-center gap-8 mb-6">
                <label class="flex items-center gap-2">
                    <input type="radio" name="role" value="siswa" {{ old('role', 'siswa') == 'siswa' ? 'checked' : '' }} class="accent-red-500">
                    <span>Siswa</span>
                </label>
                <label class="flex items-center gap-2">
                    <input type="radio" name="role" value="orangtua" {{ old('role') == 'orangtua' ? 'checked' : '' }} class="accent-red-500">
                    <span>Orang Tua</span>
                </label>
            </div>
        ...
        <p class="mt-6 text-sm">Sudah punya akun? <a href="{{ route('orangtua.login') }}" class="text-blue-600 font-semibold">Login</a></p>
