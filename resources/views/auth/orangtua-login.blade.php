<!-- resources/views/auth/orangtua-login.blade.php -->
<!DOCTYPE html>
<html lang="id">
<head>
    <title>Login Orang Tua | PT Saitama Juara Dunia</title>
...
<div class="w-1/2 flex items-center justify-center">
    <div class="w-[420px] text-center">
        ...
        <div class="flex justify-center gap-8 mb-6">
            <label class="flex items-center gap-2 cursor-pointer" onclick="window.location.href='{{ route('login') }}'">
                <input type="radio" name="role_switch" value="siswa" class="accent-red-500">
                <span>Siswa</span>
            </label>
            <label class="flex items-center gap-2 cursor-pointer">
                <input type="radio" name="role_switch" value="orangtua" checked class="accent-red-500">
                <span>Orang Tua</span>
            </label>
        </div>
        ...
        <form method="POST" action="{{ route('orangtua.login.post') }}" class="space-y-4">
            @csrf
        ...
        <p class="mt-6 text-sm">Belum punya akun? <a href="{{ route('orangtua.register') }}" class="text-blue-600 font-semibold">Daftar</a></p>
