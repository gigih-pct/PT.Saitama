    ...
    document.addEventListener('alpine:init', () => {
        Alpine.data('kesiswaan', () => ({
            ...
            async applyBatch() {
                ...
                try {
                    const response = await fetch(`/sensei/students/${this.currentStudent.id}/update-batch`, {
                        method: 'POST',
                        headers: {
            ...
            async applyStatus(option) {
                ...
                try {
                    const response = await fetch(`/sensei/students/${this.currentStudent.id}/update-status`, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
            ...
            async applyFUStatus(option) {
                ...
                try {
                    const response = await fetch(`/sensei/students/${this.currentStudent.id}/update-followup`, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
